    ...
    freePlanUploader: f({ pdf: { maxFileSize: '512MB' } })
        ...
        .onUploadComplete(async ({ metadata, file }) => {
            ...
            try {
                ...
                console.log('[PDF_PROCESSING] Parsing PDF with custom pdfjs-node wrapper...')
                // Use our wrapper to avoid DOMMatrix errors
                const { getDocument } = await import('@/lib/pdfjs-node')
                ...
                console.log(`[PDF_PROCESSING] Document loaded with ${totalPages} pages.`)
